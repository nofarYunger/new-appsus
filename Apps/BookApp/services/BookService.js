...
function filterBy(filterVal){
    let copyBooks = [...gBooks]
    return copyBooks.filter(book => {
        return book.title.toLowerCase().includes(filterVal.title.toLowerCase()) &&
            book.listPrice.amount >= filterVal.fromPrice &&
            book.listPrice.amount <= filterVal.toPrice
    })
}